...
void IRAM_ATTR x10_Check_Rcvr_wrapper() {

  //Serial.println("x10_Check_Rcvr_wrapper");
   object->Check_Rcvr((long) 800, 16, 2, (long) 10000);
  // Serial.println("x10_Check_Rcvr_wrapper done");
}

void IRAM_ATTR x10_Parse_Frame_wrapper() {
//  Serial.println("x10_Parse_Frame_wrapper");
   object->Parse_Frame();
 //  Serial.println("x10_Parse_Frame_wrapper done");
}
 ...
 */
void IRAM_ATTR x10::detectMainsFreq() {
...
*/
void IRAM_ATTR x10::write(byte houseCode, byte numberCode, int numRepeats) {

/*
  Serial.println("Write start");

  Serial.print("House code: "); Serial.println(houseCode);
  Serial.print("NumberCode code: "); Serial.println(numberCode);
  Serial.print("Repeats: "); Serial.println(numRepeats);
*/
...
void IRAM_ATTR x10::sendBits(byte cmd, byte numBits, byte isStartCode) {
...
*/
void IRAM_ATTR x10::waitForZeroCross(int pin, int howManyTimes) {
...
void IRAM_ATTR x10::Check_Rcvr(long offsetDelay, int recvPin, int ledPin, long halfCycleDelay){    // ISR - called when zero crossing (on CHANGE)
//Serial.print("Check_Rcvr");
  if (X10BitCnt == 0) {                // looking for new frame
  //Serial.print("Bit 0");
    delayMicroseconds(offsetDelay);   // wait for bit
    //Serial.print("delay done");
    if(digitalRead(recvPin)) 
        {
          //Serial.println("trying to read digital pin");
          return;  // still high - no start bit - get out
        }
  //  Serial.println("Not digital read");   
    if (ledPin>0) { digitalWrite(ledPin, HIGH); }     // indicate you got something
    //Serial.print("1");
    rcveBuff = 0;
    // Serial.print("2");
...
//    Serial.println("Bit < 5");
    delayMicroseconds(offsetDelay);   // wait for bit
    if(!digitalRead(recvPin)) rcveBuff = rcveBuff | mask;  // got a 1 set the bit, else skip and leave it 0
    mask = mask >> 1;                  // move bit down in bit mask
    X10BitCnt++;

    if(X10BitCnt == 13){               // done with frame after 13 bits
   // Serial.print("Bits 13");
      for (byte i=0;i<5;i++) delayMicroseconds(halfCycleDelay); // need this
      X10rcvd = true;                  // a new frame has been received
    //  Serial.print("Got inner message");
      if (ledPin>0) { digitalWrite(ledPin, LOW); }     // indicate you got something
      X10BitCnt = 0;
      x10_Parse_Frame_wrapper();                   // parse out the house & unit code and command
    }
  }
  if (ledPin>0) { digitalWrite(ledPin, LOW); } 
 // Serial.print("Bit end");
}

#if 1
void IRAM_ATTR x10::Parse_Frame() {   // parses the receive buffer to get House, Unit, and Cmnd
//Serial.print("Old Parse_Frame");
...
// NEW VERSION - better error checking
 void IRAM_ATTR Parse_Frame() {   // parses the receive buffer to get House, Unit, and Cmnd
 //Serial.print("New Parse_Frame");
...
void x10::debug_print(char* outStr)
{

  const char *stCode = reinterpret_cast<const char*>(startCode);
  const char *houseCode = reinterpret_cast<const char*>(_houseCode);
  const char *unitCode = reinterpret_cast<const char*>(_unitCode);
  const char *cmndCode = reinterpret_cast<const char*>(_cmndCode);
  const char *space = " ";

  char result[100];

  strcpy(result,stCode);
  strcpy(result,space);
  strcpy(result,houseCode);
  strcpy(result,space);
  strcpy(result,unitCode);
  strcpy(result,space);
  strcpy(result,cmndCode);
